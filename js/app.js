    ...
    <div class="occ-card ${started ? 'occ-started' : ''}" onclick="startTriage('${occ.id}')">
      <div class="occ-card-top">
        ...
        ${started ? `<div class="occ-ring" title="${pct}% concluído">
          <svg viewBox="0 0 36 36"><circle cx="18" cy="18" r="15.9" fill="none" stroke="var(--border)" stroke-width="2.5"/><circle cx="18" cy="18" r="15.9" fill="none" stroke="var(--accent)" stroke-width="2.5" stroke-dasharray="${pct} ${100 - pct}" stroke-dashoffset="25" stroke-linecap="round"/></svg>
          <span>${pct}%</span>
        </div>` : ''}
      </div>
